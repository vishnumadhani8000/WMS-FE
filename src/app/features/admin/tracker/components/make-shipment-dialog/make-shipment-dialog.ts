...
import { AvailableDriverDto, AvailableVehicleDto, MakeShipmentDialogData, MakeShipmentDialogResult } from '../../models/tracker.model';
...
export class MakeShipmentDialog implements OnInit {
  ...
  selectDriver(driver: AvailableDriverDto): void {
    this.selectedDriverId.set(
      this.selectedDriverId() === driver.id ? null : driver.id
    );
  }

  selectVehicle(vehicle: AvailableVehicleDto): void {
    this.selectedVehicleId.set(
      this.selectedVehicleId() === vehicle.id ? null : vehicle.id
    );
  }
